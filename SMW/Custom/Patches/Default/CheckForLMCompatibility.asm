...
;---------------------------------------------------------------------------
	...
	!USAOrgLoc = $00AF4B : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
		%PrintLMLabelOffset(SMW_HandlePaletteFades, LM170Hijack_FadeFix1)

	!USAOrgLoc = $00AF4E : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
		%PrintLMLabelOffset(SMW_HandlePaletteFades, LM170Hijack_FadeFix2)

	!USAOrgLoc = $00AF6B : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
		%PrintLMLabelOffset(SMW_HandlePaletteFades, LM170Hijack_FadeFix3)

	!USAOrgLoc = $00AF74 : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
		%PrintLMLabelOffset(SMW_HandlePaletteFades, LM170Hijack_FadeFix4)
...
;---------------------------------------------------------------------------
	...
	!HACK_SMW_brOrgLoc = !USAOrgLoc
		%PrintLMLabelOffset(SMW_PlayerState00_Normal, LM253Hijack_VictoryPoseTimingFix)
...
;---------------------------------------------------------------------------
	...
	!USAOrgLoc = $048E49 : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
		%PrintLMLabelOffset(SMW_GameMode0C_LoadOverworld, LM220Hijack_SetKoopaTeleportHere1)

	!USAOrgLoc = $048E4F : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
		%PrintLMLabelOffset(SMW_GameMode0C_LoadOverworld, LM220Hijack_SetKoopaTeleportHere2)
...
;---------------------------------------------------------------------------
	...
	!HACK_SMW_brOrgLoc = !USAOrgLoc
		%PrintLMLabelOffset(SMW_GameMode0C_LoadOverworld, LM000Hijack_CustomLevelNames1)

	!USAOrgLoc = $049549 : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
	!HACK_SMW_brOrgLoc = !USAOrgLoc
		%PrintLMLabelOffset(SMW_OverworldProcess04_PlayerIsMoving, LM000Hijack_CustomLevelNames2)
...
;---------------------------------------------------------------------------
	...
	!HACK_SMW_brOrgLoc = !USAOrgLoc
		%PrintLMLabelOffset(SMW_LoadOverworldLayer1AndEvents, LM000Hijack_PlaceOverworldLevelAnywhere)
...
;---------------------------------------------------------------------------
	...
	!USAOrgLoc = $00A439 : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
	!HACK_SMW_brOrgLoc = !USAOrgLoc
		%PrintLMLabelOffset(SMW_RestoreSP1AfterMarioStart, LM221Hijack_DisableSP1VRAMBackup1)

;---------------------------------------------------------------------------

	; Info: This pointer is hijacked to allow you to have a custom layer 3 title screen image. (loc + $0084C8)
	!USAOrgLoc = $000003 : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
	!HACK_SMW_brOrgLoc = !USAOrgLoc
		%PrintLMLabelOffset(SMW_LoadStripeImage, LM000Hijack_CustomTitleScreen)

;---------------------------------------------------------------------------

	; Info: Lunar Magic hijacks all the pointers to the castle destruction text so you can have custom text for these cutscenes (loc + $0084C8)
	; Note that these hijacks don't apply to the Japanese version.
	!USAOrgLoc = $000021 : !JAPANOrgLoc = NULLROM : !SMASUOrgLoc = NULLROM
	!HACK_SMW_brOrgLoc = !USAOrgLoc
